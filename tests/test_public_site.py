...
class PublicSiteTests(unittest.TestCase):
    def test_mobile_navigation_keeps_brand_and_controls_on_one_row(self):
        ...
        self.assertIn("grid-template-columns: max-content minmax(0, 1fr);", site_css)
        self.assertNotIn("grid-template-columns: minmax(0, 1fr) auto minmax(0, 1fr);", site_css)
        self.assertNotIn('class="site-links"', index_html)
        ...
        self.assertNotIn("transform: translate(-50%, -50%);", site_css)
        self.assertEqual(index_html.count('class="site-menu-separator'), 3)
        self.assertLess(index_html.index('id="visitorCount"'), index_html.index('id="traceTrigger"'))
        self.assertLess(index_html.index('id="traceTrigger"'), index_html.index('id="contactTrigger"'))
        self.assertLess(index_html.index('id="contactTrigger"'), index_html.index('id="filterTrigger"'))
    ...
    def test_lightbox_keeps_metadata_and_tabs_without_map_action(self):
        ...
        self.assertRegex(site_css, r"\.lightbox-tab \{[^}]*border: 0;[^}]*background: transparent;")
        self.assertIn(".lightbox-tab.is-active::after", site_css)
        self.assertRegex(site_css, r"\.lightbox-close \{[^}]*border: 0;")
        self.assertRegex(site_css, r"\.trace-close \{[^}]*border: 0;")
        self.assertIn("border-top: 0;", site_css)
